	...
	IN PNDIS_802_11_VARIABLE_IEs pVIE)
{
	...
	int displayMsg = 0;
	/*
	if(NdisEqualMemory(pBssid, "\x00\x70\x06\x12\x3b\x24", 6))
		displayMsg = 1;
	*/		
							
	if(displayMsg){
		Ssid[SsidLen] = '\0';
		printk("%s(): SSID(%d): [%s] [%02x:%02x:%02x:%02x:%02x:%02x]\n",
			__FUNCTION__, SsidLen, Ssid,
	...
	if(displayMsg) printk("%s(): LINE: %d Exits\n", __FUNCTION__, __LINE__);
	return Idx;
}
