...
int main(int argc, char *argv[])
{
	...
	{/* NOTE: Max Yin: close all file descriptors after 2 */
		int i;
		for(i=3 ; i<1024 ; i++){
			close(i);
		}
	...
	{ /* NOTE: Max Yin Stardate 10706.1 
		Just write the disk label to /tmp for UI to read. UI doesn't need to call libntfs to get this
	   */
		char buf[128],*slash;
		slash = strrchr(opts.device, '/');
		if(slash!=NULL)
			slash ++;
		else
			slash = opts.device;
		sprintf(buf, "/tmp/ntfs_label.%s", slash);
		FILE *Flabel = fopen(buf, "w+");
		if(Flabel!=NULL){
			fprintf(Flabel, "%s", ctx->vol->vol_name);
			fclose(Flabel);
		}
	}
