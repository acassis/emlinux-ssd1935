...
// 2007.02.01 : only big endian system need to do this
#if defined(MIPS_RTL865xB)
#define WORDS_BIGENDIAN	1
#endif

#ifndef WORDS_BIGENDIAN
static uint32_t swapu32(uint32_t n)
{
	return (    ((n & 0xffU) << 24) |
				((n & 0xff00U) << 8) |
				((n & 0xff0000U) >> 8) |
...
void Blowfish_Init(BLOWFISH_CTX *ctx, unsigned char *key, int keyLen) {
  ...
  for (i = 0; i < N + 2; i += 2) {
    ...
    ctx->P[i + 1] = datar;
	#else
	ctx->P[i] = swapu32(datal);
    ctx->P[i + 1] = swapu32(datar);
	#endif
  }
  ...
  for (i = 0; i < 4; ++i) {
    for (j = 0; j < 256; j += 2) {
      ...
      ctx->S[i][j + 1] = datar;
	  #else
	  ctx->S[i][j] = swapu32(datal);
      ctx->S[i][j + 1] = swapu32(datar);
	  #endif
    }
  }
}
...
// The following code comes from the beecyrpt-4.1.2

void Blowfish_SetIV(BLOWFISH_CTX *ctx, const unsigned char* iv)
{
	if (iv)
		memcpy(ctx->feedback, iv, 8);
	else
		memset(ctx->feedback, 0, 8);
}

int Blowfish_blockEncryptCBC(BLOWFISH_CTX *ctx, 
							 uint32_t* dst, 
							 const uint32_t* src, 
							 unsigned int nblocks)
{
	register const unsigned int blockwords = 2; // 8 bytes
	register uint32_t* fdback = ctx->feedback;
	...
	if (nblocks > 0)
	{
		...
		Blowfish_Encrypt(ctx, dst, dst+1);
...
int Blowfish_blockDecryptCBC(BLOWFISH_CTX *ctx, 
							 uint32_t* dst, 
							 const uint32_t* src, 
							 unsigned int nblocks)
{
	...
	if (buf)
	{
		while (nblocks > 0)
		{
			...
			memcpy(buf, src, blockwords*sizeof(uint32_t));
