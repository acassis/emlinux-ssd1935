...
// 2007.12.14 : add the rijndael optimized C code v3.0 support
//				for performance issue (it can speed up to 0.5X)

#ifndef USE_RIJNDAEL_CODE
...
void aes_set_key( aes_context *ctx, uchar *key, int keysize )
{
	rijndaelKeySetupEnc((u32 *)&ctx->erk[0], key, keysize);
	rijndaelKeySetupDec((u32 *)&ctx->drk[0], key, keysize);
	ctx->nr = (keysize >> 5) + 6;
}
...
void aes_encrypt( aes_context *ctx, uchar input[16], uchar output[16] )
{
	rijndaelEncrypt((u32 *)&ctx->erk[0], ctx->nr, input, output);
}
...
void aes_decrypt( aes_context *ctx, uchar input[16], uchar output[16] )
{
	rijndaelDecrypt((u32 *)&ctx->drk[0], ctx->nr, input, output);
}
