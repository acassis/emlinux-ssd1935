...
extern StreamIO *mplib_streamio_p;
extern void *mplib_data_p;

/******************************/
/*      Static functions           */
/******************************/

#ifndef DISABLE_ID3_V1
...
id3v1_get_tag(int fd)
{
	...
	// 2005.03.23 Sky add : modify if mplib_streamio_p is not NULL, use external streamIO functions
	// seek to the start of stream
	if (mplib_streamio_p != NULL)
	{
		if (mplib_streamio_p->streamio_seek(mplib_data_p,-128L, SEEK_END) == -1) goto exit_on_error;
	}
	else
	{
		if(lseek(fd, -128L, SEEK_END) == -1) goto exit_on_error;
	}
	// 2005.03.23 End of add
	
	// 2005.03.23 Sky add : modify if mplib_streamio_p is not NULL, use external streamIO functions
	// seek to the start of stream
	if (mplib_streamio_p != NULL)
	{
		if (mplib_streamio_p->streamio_read(mplib_data_p, c, 3, 0) < 3) goto exit_on_error;
	}
	else
	{
	...
	if (mplib_streamio_p != NULL)
	{
		if (mplib_streamio_p->streamio_read(mplib_data_p, tag->title, 30, 0) < 30) goto exit_on_error;
	}
	else
	{
	...
	if (mplib_streamio_p != NULL)
	{
		if (mplib_streamio_p->streamio_read(mplib_data_p, tag->artist, 30, 0) < 30) goto exit_on_error;
	}
	else
	{
	...
	if (mplib_streamio_p != NULL)
	{
		if (mplib_streamio_p->streamio_read(mplib_data_p, tag->album, 30, 0) < 30) goto exit_on_error;
	}
	else
	{
	...
	if (mplib_streamio_p != NULL)
	{
		if (mplib_streamio_p->streamio_read(mplib_data_p, tag->year, 4, 0) < 4) goto exit_on_error;
	}
	else
	{
	...
	if (mplib_streamio_p != NULL)
	{
		if (mplib_streamio_p->streamio_read(mplib_data_p, tag->comment, 30, 0) < 30) goto exit_on_error;
	}
	else
	{
	...
	if (mplib_streamio_p != NULL)
	{
		if (mplib_streamio_p->streamio_read(mplib_data_p, &(tag->genre), 1, 0) < 1) goto exit_on_error;
	}
	else
	{
...
#endif // #ifndef DISABLE_ID3_V1

#ifndef OPTIMIZE_SRC_CODE_SIZE
...
#endif // #ifndef OPTIMIZE_SRC_CODE_SIZE
...
id3v2_get_tag(int fd)
{
	...
	if (mplib_streamio_p != NULL)
	{
		if (mplib_streamio_p->streamio_seek(mplib_data_p, 0L, SEEK_SET) == -1) return NULL;
	}
	else
	{
	...
	if (mplib_streamio_p != NULL)
	{
		if(mplib_streamio_p->streamio_read(mplib_data_p, c, 10, 0) < 10)
			goto exit_on_error;
	}
	else
	{
	...
	/// 2005.03.31 Sky modify : id3v2.2 support
	if (header->version_minor == 2)
		header->has_compression = (c[5] & 64) >> 6;
	...
	if(header->has_extended_header)
	{
		...
		// 2005.03.23 Sky add : 
		// modify if mplib_streamio_p is not NULL, 
		// use external streamIO functions
		if (mplib_streamio_p != NULL)
			mplib_streamio_p->streamio_read(mplib_data_p, c, 4, 0);
		else read(fd, c, 4); /* get length of extended header */
		// 2005.03.23 End of add
		...
		// use external streamIO functions
		if (mplib_streamio_p != NULL)
			mplib_streamio_p->streamio_read(mplib_data_p, c, 1, 0);
		else read(fd, c, 1); /* get number of flags */
		// 2005.03.25 End of add
		...
		// use external streamIO functions
		if (mplib_streamio_p != NULL)
			mplib_streamio_p->streamio_read(mplib_data_p, c, xt_header->no_flag_bytes, 0);
		else read(fd, c, xt_header->no_flag_bytes); /* get flag bytes */
		// 2005.03.25 End of add
		...
		/* Flag data */
		if(xt_header->is_update)
		{
			// 2005.03.23 Sky add : modify if mplib_streamio_p is not NULL, use external streamIO functions
			if (mplib_streamio_p != NULL)
				mplib_streamio_p->streamio_read(mplib_data_p, c, 1, 0);
			else read(fd, c, 1); /* Data length ind. is 0 -skip */
			// 2005.03.23 End of add
		}
		...
		if(xt_header->crc_data_present) {
			...
			if (mplib_streamio_p != NULL)
				mplib_streamio_p->streamio_read(mplib_data_p, c, 1, 0);
			else read(fd, c, 1); /* data length - shoud be 5 */
			// 2005.03.23 End of add
			if(*c != 5)	goto exit_on_error; /*  else things might break badly */
			...
			// use external streamIO functions
			if (mplib_streamio_p != NULL)
				mplib_streamio_p->streamio_read(mplib_data_p, xt_header->crc_data, *c, 0);
			else read(fd, xt_header->crc_data, *c);
			// 2005.03.23 End of add
		}
		if(xt_header->restrictions) {
			...
			if (mplib_streamio_p != NULL)
				mplib_streamio_p->streamio_read(mplib_data_p, c, 1, 0);
			else read(fd, c, 1); /* data length - shoud be 1 */
			// 2005.03.23 End of add
			...
			// use external streamIO functions
			if (mplib_streamio_p != NULL)
				mplib_streamio_p->streamio_read(mplib_data_p, xt_header->restrictions_data, *c, 0);
			else read(fd, xt_header->restrictions_data, *c); 
			// 2005.03.23 End of add
		}
	}
	...
	while(1)
	{
		...
		if (mplib_streamio_p != NULL)
		{
			seek_ret = mplib_streamio_p->streamio_seek(
						mplib_data_p, 0L, SEEK_CUR);
			if (seek_ret >= header->total_tag_size) break;
			if (seek_ret == -1) goto exit_on_error;
		}
		else
		{
			seek_ret = lseek(fd, 0L, SEEK_CUR);
		...
		if (header->version_minor == 2)
		{
			...
			// use external streamIO functions
			if (mplib_streamio_p != NULL)
				read_ret = mplib_streamio_p->streamio_read(
							mplib_data_p, c, 6, 0);
			else read_ret = read(fd, c, 6); /* Read header */
			// 2005.03.23 End of add
			
			if (read_ret != 6) goto exit_on_error;
			
			/* break if padding is reached - this should never happen here.. */
			if(c[0] == 0 && c[1] == 0 && c[2] ==  0) break;
			/* Check if possible id is alpha numeric */
			if(!isalnum(c[0]) || !isalnum(c[1]) || !isalnum(c[2])) break;
			frame = XMALLOCD(id3v2_frame, "id3v2_get_tag:frame");
			frame->frame_id = xmallocd(5, "id3v2_get_tag:frame->frame_id");
			strncpy(frame->frame_id, c, 3);
			frame->frame_id[3] = '\0';
			frame->data_size = c[5] + (c[4] << 8) + (c[3] << 16);
		
		}//  if ((header->version_minor == 3) || (header->version_minor == 4))
		else
		{
			// 2005.03.23 Sky add : 
			// modify if mplib_streamio_p is not NULL, 
			...
			if (mplib_streamio_p != NULL)
				read_ret = \
					mplib_streamio_p->streamio_read(mplib_data_p, c, 10, 0);
			else read_ret = read(fd, c, 10); /* Read header */
			// 2005.03.23 End of add
			
			if (read_ret != 10) goto exit_on_error;
			/* break if padding is reached - this should never happen here.. */
			if(c[0] == 0 && c[1] == 0 && c[2] ==  0 && c[3] == 0) break;
			
			// 2010.02.02 :
			// MP3ext tagging tool add advertisement at the end of id3 tag
			// we should skip it
			if (strncasecmp(c, "MP3ext V", 8) == 0)
			{
				printf("%s(): we found MP3ext tag tool advertisment\n", __FUNCTION__);
				
				// seek to the end of tag
				if (mplib_streamio_p != NULL)
					mplib_streamio_p->streamio_seek(mplib_data_p, header->total_tag_size, SEEK_SET);
				else lseek(fd, header->total_tag_size, SEEK_SET);
				break;
			}
			//
			...
			frame->frame_id[4] = '\0';
			// 2008.11.14 : 
			// for 2.3 version, frame size is not unsync32
			// for 2.4 version, frame size is unsync32
			// 2005.03.04 Sky Huang modify : caculate data size with normal way
			if (header->version_minor == 4)
				frame->data_size = id3_unsync32(c, 4);
			else frame->data_size = c[7] + (c[6] << 8) + (c[5] << 16) + (c[4] << 24);
			// 2005.03.04 End of add
			frame->status_flag = c[8];
			frame->format_flag = c[9];
		}
		
		/* Getting frame content */
		frame->data = xmallocd(frame->data_size + 1, "id3v2_get_tag:frame->data_size");
		
		// 2006.08.01 remember the stream offset for future use
		if (mplib_streamio_p != NULL)
			frame->stream_offset = \
				mplib_streamio_p->streamio_seek(mplib_data_p, 0L, SEEK_CUR);
		else frame->stream_offset = lseek(fd, 0L, SEEK_CUR);
		
		char *data = frame->data;
		unsigned int data_size = frame->data_size;
		unsigned int evl_data_offset = 0;
...
unsyncronization:
		...
		{
			// 2008.01.03 :
			// if frame data is picture, it may very large one.
			// we need to confirm that the whole image is readed
			// TODO :
			// pass the job of reading to caller and let caller
			// to decide the best way of reading
			
			read_ret = mplib_streamio_p->streamio_read(
						mplib_data_p, data, data_size, 0);
			//
		}
		else read_ret = read(fd, data, data_size);
		if (read_ret != data_size) goto exit_on_error;
		
		// 2009.03.10 :
		// for unsynchronisation scheme, 
		// we remove padded data
		if (header->unsyncronization && data_size > 2)
		{
			int i;
			int skipped_bytes = 0;
			char *pSrcData = data - evl_data_offset;
			char *pDstData = data - evl_data_offset;
			
			for(i = 0; i < data_size - 2 + evl_data_offset; i++)
			{
				while(pSrcData[0] == 0xFF && 
					  pSrcData[1] == 0x00 && 
					  (pSrcData[2] == 0x00 || pSrcData[2] >= 0xE0))
				{
					*pDstData++ = pSrcData[0];
					pSrcData+=2;
					i+=2;
			...
			*pDstData++ = *pSrcData++;
			...
			if (skipped_bytes > 0)
			{
				printf("%s(): we need read more %d bytes\n", 
						__FUNCTION__, skipped_bytes);
				evl_data_offset = 2;
				data = pDstData;
				data_size = skipped_bytes;
				goto unsyncronization;
			}
		}
		//
	...
	printf("%s(): exit_on_error\n", __PRETTY_FUNCTION__);
	xfree(c);
	id3v2_free_tag(tag);
	return NULL;
}

// 2005.03.23 Sky : mark off we don't need these functions
#ifndef OPTIMIZE_SRC_CODE_SIZE
