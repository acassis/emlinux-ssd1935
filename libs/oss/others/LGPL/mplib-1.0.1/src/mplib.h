...
// 2005.03.23 Sky add : add an function suite for I/O reading and out
//			and use an registeration way for registering these I/O functions
//			default I/O functions is file I/O
typedef struct _StreamIO {
	char StreamIOFnSuiteName[64];
	int (*streamio_open)(void *Content_p, int flags);
	int (*streamio_read)(void *Content_p, char *buffer_p, int size, int type);
	signed long long (*streamio_seek)(void *Content_p, signed long long offset, int whence);
	int (*streamio_close)(void *Content_p);
}StreamIO;
...
typedef struct _id3v2_frame
{
  ...
  unsigned int data_size; /* frame size excluding header, incl. enc.,lang.,etc.
				  (total frame size - 10) for id3v2.3.0 upper
				  (total frame size -  6) for id3v2.2.0  */
  unsigned int stream_offset; /* stream offset to current frame data 2006.08.01 added */
} id3v2_frame;
...
typedef struct _id3v2_header
{
  ...
  unsigned int has_compression;	// 2004.03.31 Sky add : id3v2.2 field
...
//			and use an registeration way for registering these I/O functions
//			default I/O functions is file I/O
extern int mp_register_streamio_fn_suite __P((StreamIO*, void*));
// 2005.03.23 End of add
