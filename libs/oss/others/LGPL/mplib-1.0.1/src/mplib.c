...
// 2005.03.23 Sky add : static internal StreamIO
StreamIO *mplib_streamio_p = NULL;
void *mplib_data_p = NULL;
...
// 2005.03.23 Sky add : using for registering the streamio fuction from outside
int mp_register_streamio_fn_suite(StreamIO *streamio_p, void *data_p)
{
	// assign the operations
	if (streamio_p != NULL) mplib_streamio_p = streamio_p;
	else return 0;
	
	// assign the content data
	if (data_p != NULL) mplib_data_p = data_p;
	else return 0;
...
mp_get_tag_list_from_file(const char* filename)
{
	...
	// 2005.03.23 Sky add : modify if mplib_streamio_p is not NULL, use external Stream I/O functions
	if (mplib_streamio_p != NULL)
		fd = mplib_streamio_p->streamio_open(mplib_data_p, O_RDONLY);
	else
		fd = open(filename, O_RDONLY);
	// 2005.03.23 End of add
	...
	// 2005.03.23 Sky add : modify if mplib_streamio_p is not NULL, use external Stream I/O functions
	if (mplib_streamio_p != NULL)
		mplib_streamio_p->streamio_close(mplib_data_p);
	else
		close(fd);
	// 2005.03.23 End of add
...
mp_get_tag_list_from_fd(int fd)
{
	...
	if(v2tag)
	{
		...
		// 2005.03.31 Sky add : id3v2.2 support
		if(v2tag->header->version_minor == 2 || 
		   v2tag->header->version_minor == 3 || 
		   v2tag->header->version_minor == 4)
			tag->version = 2;
		else
			tag->version = -1;
		tag->tag = v2tag;
	...
	// 2005.12.13 Sky modified for our target
	// we don't need the id3 v1 tag parsing
