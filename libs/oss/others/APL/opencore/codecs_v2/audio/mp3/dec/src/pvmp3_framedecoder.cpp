...
///#define EVL_DECODE_SPEED

#ifdef EVL_DECODE_SPEED
...
static struct timeval tv_fst;
static struct timeval tv_snd;

static int GetDiffTime(struct timeval *tv_fst, struct timeval *tv_snd)
{
	unsigned long long bits_snd = 0;
	int diffmicrosnd = 0;
	
	if (tv_snd->tv_usec >= tv_fst->tv_usec)
		diffmicrosnd = \
			(tv_snd->tv_sec - tv_fst->tv_sec) * 1000000 + \
			(tv_snd->tv_usec - tv_fst->tv_usec);
	else
		diffmicrosnd = \
			(tv_snd->tv_sec - tv_fst->tv_sec - 1) * 1000000 + \
			(tv_snd->tv_usec + 1000000) - tv_fst->tv_usec;
	return diffmicrosnd;
}
#endif
                              ...
                              void              *pMem)
{
	...
	printf("predication %d micro-sec\n", GetDiffTime(&tv_fst, &tv_snd));
#endif
...
#ifdef EVL_DECODE_SPEED
		gettimeofday(&tv_snd, NULL);
		printf("get side info %d micro-sec\n", GetDiffTime(&tv_fst, &tv_snd));
		gettimeofday(&tv_fst, NULL);
#endif
        /*
...
#ifdef EVL_DECODE_SPEED
		gettimeofday(&tv_snd, NULL);
		printf("get main data %d micro-sec\n", GetDiffTime(&tv_fst, &tv_snd));
#endif
...
#ifdef EVL_DECODE_SPEED
			gettimeofday(&tv_snd, NULL);
			printf("scalefactors+dequantize %d micro-sec\n", GetDiffTime(&tv_fst, &tv_snd));
			gettimeofday(&tv_fst, NULL);
#endif
            for (ch = 0; ch < pVars->num_channels; ch++)
            {
...
#ifdef EVL_DECODE_SPEED
			gettimeofday(&tv_snd, NULL);
			printf("reorder+alias_reduction+imdctphase_synthesis %d micro-sec\n", 
					GetDiffTime(&tv_fst, &tv_snd));
			gettimeofday(&tv_fst, NULL);
#endif
