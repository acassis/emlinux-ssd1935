...
// 2009.12.28 :
// return sbrPresentFlag in structure MC_Info
Int PVMP4GetSBRPresentFlag(void *pMem)
{
	if (pMem == NULL) return 0;
	
	tDec_Int_File *pVars = (tDec_Int_File *)pMem;
	MC_Info *pMC_Info = &pVars->mc_info;
///	printf("%s(): sbrPresentFlag:%d\n", __FUNCTION__, pMC_Info->sbrPresentFlag);
	return pMC_Info->sbrPresentFlag;
}

void PVMP4SetSBRPresentFlag(void *pMem, Int value)
{
	if (pMem == NULL) return;
	...
	MC_Info *pMC_Info = &pVars->mc_info;
///	printf("%s(): sbrPresentFlag:%d\n", __FUNCTION__, pMC_Info->sbrPresentFlag);
	pMC_Info->sbrPresentFlag = value;
}
//

// return psPresentFlag in structure MC_Info
Int PVMP4GetPSPresentFlag(void *pMem)
{
	if (pMem == NULL) return 0;
	
	tDec_Int_File *pVars = (tDec_Int_File *)pMem;
	MC_Info *pMC_Info = &pVars->mc_info;
	return pMC_Info->psPresentFlag;
}
//

void PVMP4SetAACPlus(void *pMem, bool value)
{
	if (pMem == NULL) return;
	
	tDec_Int_File *pVars = (tDec_Int_File *)pMem;
	pVars->aacPlusEnabled = value;
}
#if 0
void PVADTSSetOn(void *pMem)
{
	if (pMem == NULL) return;
...
#endif
// 2010.01.11 :
// set pVars->bno to some value
// sometimes when we meet some corrupted packets
// we always call the PVMP4AudioDecoderResetBuffer()
// But the value of pVars->bno is always set to 1.
// we need to set it to 2 to info the PVMP4AudioDecodeFrame()
// that we are not started from the first 2 frames
void PVMP4SetBNO(void *pMem, UInt32 bno)
{
	if (pMem == NULL) return;
...
// 2010.01.19 :
// enable Implicit_channeling for the first CPE or SCE element
void PVMP4SetImplicit_channeling(void *pMem, Int implicit_channeling)
{
	if (pMem == NULL) return;
...
Int PVMP4GetNch(void *pMem)
{
	if (pMem == NULL) return 0;
	
	tDec_Int_File *pVars = (tDec_Int_File *)pMem;
	return pVars->mc_info.nch;
}

Int PVMP4GetSampleRate(void *pMem)
{
	if (pMem == NULL) return 0;
	...
	if (pVars->prog_config.sampling_rate_idx <= 12 && 
		pVars->prog_config.sampling_rate_idx >= 0)
		return SRIdxTable[pVars->prog_config.sampling_rate_idx];
