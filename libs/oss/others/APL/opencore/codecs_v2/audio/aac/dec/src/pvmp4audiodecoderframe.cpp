    ...
    void                        *pMem)
{
    ...
    // 2009.12.28 :
	// check if sbr extra data is presented
...
#ifdef AAC_PLUS
				// 2009.12.28 :
				// return "sbrPresentFlag" once ext sbr is presented
                sbrPresentFlag |= get_sbr_bitstream(sbrBitStream, &pVars->inputStream);
				//
#else
...
// 2010.01.26 :
// save the sbrPresentFlag to pMC_Info
// once sbrPresentFlag is found, set aacPlusEnabled to ON state
	if (!pMC_Info->sbrPresentFlag)
	    pMC_Info->sbrPresentFlag = sbrPresentFlag;
	if (pMC_Info->sbrPresentFlag)
	{
		if (!pExt->aacPlusEnabled) printf("enable aacPlus\n");
...
#endif
///	printf("sbrBitStream->NrElements:%d\n", sbrBitStream->NrElements);
    /*
...
///			printf("pVars->bno:%d\n", pVars->bno);
            if (pVars->bno <= 1)   /* allows console to operate with ADIF and audio config */
            {
				...
				printf("sbrDec->outSampleRate:%d, pMC_Info->nch:%d\n", sbrDec->outSampleRate, pMC_Info->nch);
            }
        ...
        if (pVars->bno <= 1)
        {
            ...
            if (pMC_Info->upsamplingFactor == 2)
            {
				...
				printf("%s(): update info by upsamplingFactor\n", __FUNCTION__);
            }
