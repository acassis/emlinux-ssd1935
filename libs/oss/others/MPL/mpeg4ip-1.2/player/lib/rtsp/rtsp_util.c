...
void clear_decode_response (rtsp_decode_t *resp)
{
  ...
  // #ifdef ENABLE_WMA
  CHECK_AND_FREE(resp->x_playlist_gen_id);	// 2005.04.06 Sky add : for microsoft ms rtsp
  CHECK_AND_FREE(resp->x_broadcast_id);		// 2005.04.06 Sky add : for microsoft ms rtsp
  // #endif
  // #ifdef ENABLE_RA
  CHECK_AND_FREE(resp->realchallenge1);		// 2005.08.16 Sky add : for realaudio ra rtsp
  CHECK_AND_FREE(resp->statsmask);		// 2005.08.16 Sky add : for realaudio ra rtsp
  CHECK_AND_FREE(resp->etag);			// 2005.08.16 Sky add : for realaudio ra rtsp
  // #endif
