				     ...
				     unsigned short rtp_len)
{
  ...
  if (info->m_callback[interleave].rtp_callback_set) {
    ...
    // 2005.04.25 Sky modify : use free() to replace the xfree() for prevent use rtp library currently
    #if 0
    xfree(rtp_ptr);
    #else
    free(rtp_ptr);
...
static int get_rtp_packet (rtsp_client_t *info, rtp_state_t *state)
{
  ...
  if (state->rtp_ptr == NULL) {
    ...
    // 2005.04.25 Sky modify : use malloc() to replace the xmalloc() for prevent use rtp library currently
    #if 0
    state->rtp_ptr =
      (rtp_packet *)xmalloc(state->rtp_len + RTP_PACKET_HEADER_SIZE);
    #else
    state->rtp_ptr =
      (rtp_packet *)malloc(state->rtp_len + RTP_PACKET_HEADER_SIZE);
    #endif
    // 2005.04.25 End of modification
...
int rtsp_thread (void *data)
{
  ...
  // 2005.04.25 Sky mark off : currently we don't use the SDL library
  #if 0
  SDL_Delay(10);
  #endif
  // 2005.04.25 End of mark off
