...
// 2005.04.25 Sky mark off : we don't need math.h library
#if 0
#include <math.h>
#endif
...
#define SNPRINTF_CHECK(fmt, value) \
  ...
  *at += ret;
  ...
  /// 2005.03.31 Sky modify : scale type change to the char array, use the strncasecmp
  ///if (cmd && cmd->scale != 0.0) {
  if (cmd && (strncasecmp(cmd->scale, "0.0", strlen("0.0")) != 0)) {
    ///SNPRINTF_CHECK("Scale: %f\r\n", cmd->scale);
    SNPRINTF_CHECK("Scale: %s\r\n", cmd->scale);
  /// End of modify
  }
  
  /// 2005.03.31 Sky modify : scale type change to the char array, use the strncasecmp
  ///if (cmd && cmd->speed != 0.0) {
  if (cmd && (strncasecmp(cmd->speed, "0.0", strlen("0.0")) != 0)) {
    ///SNPRINTF_CHECK("Speed: %f\r\n", cmd->speed);
    SNPRINTF_CHECK("Speed: %s\r\n", cmd->speed);
    /// End of modify
  }
  if (cmd && cmd->useragent) {
...
/*
 * rtsp_send_set_parameter - send the set parameter command to a server
 */
			     ...
			     rtsp_decode_t **decode_result)
{
  ...
  *decode_result = NULL;
  ...
  do {
    maxlen = sizeof(buffer);
    buflen = snprintf(buffer, maxlen, "SET_PARAMETER %s RTSP/1.0\r\n", info->url);
    ...
    rtsp_debug(LOG_INFO, "Sending SET_PARAMETER %s", info->url);
    rtsp_debug(LOG_DEBUG, "%s", buffer);
    ...
    if (ret == RTSP_RESPONSE_GOOD) {
      rtsp_debug(LOG_INFO, "SET_PARAMETER returned correctly");
      *decode_result = info->decode_response;
      info->decode_response = NULL;
      return (RTSP_RESPONSE_GOOD);
    } else if (ret != RTSP_RESPONSE_REDIRECT) {
      
      rtsp_debug(LOG_ERR, "SET_PARAMETER return code %d", ret);
      if (ret != RTSP_RESPONSE_RECV_ERROR &&
	  decode != NULL) {
	*decode_result = info->decode_response;
	info->decode_response = NULL;
...
// 2005.04.15 Sky add : add one more method GET_PARAMETER for RTSP ACK
/*
 * rtsp_send_get_parameter - send the GET_PARAMETER info to a server
 */
			     ...
			     rtsp_decode_t **decode_result)
{
  ...
  *decode_result = NULL;
  info->redirect_count = 0; // for counting the redirect times

  do {
    maxlen = sizeof(buffer); /*GET_PARAMETER*/
    buflen = snprintf(buffer, maxlen, "GET_PARAMETER %s RTSP/1.0\r\n", info->url);

    if (rtsp_build_common(buffer, maxlen, &buflen, info, cmd, NULL) == -1) { // use the cmd session
      return (RTSP_RESPONSE_RECV_ERROR);
    }
    ...
    rtsp_debug(LOG_INFO, "Sending GET_PARAMETER %s", info->url);
    rtsp_debug(LOG_DEBUG, "%s", buffer);
    
    return rtsp_send(info, buffer, buflen);
    ...
    if (ret == RTSP_RESPONSE_GOOD) {
      rtsp_debug(LOG_INFO, "GET_PARAMETER returned correctly");
      *decode_result = info->decode_response;
      info->decode_response = NULL;
      return (RTSP_RESPONSE_GOOD);
    } else if (ret != RTSP_RESPONSE_REDIRECT) {
      rtsp_debug(LOG_ERR, "GET_PARAMETER return code %d", ret);
      if (ret != RTSP_RESPONSE_RECV_ERROR &&
	  decode != NULL) {
	*decode_result = info->decode_response;
	info->decode_response = NULL;
...
// 2005.04.15 End of addition

// 2005.08.15 Sky add : add one more method OPTIONS for RTSP ACK
/*
 * rtsp_send_options - send the OPTIONS info to a server
 */

int rtsp_send_options (rtsp_client_t *info, \
		       rtsp_command_t *cmd, \
		       rtsp_decode_t **decode_result)
{
  char buffer[2048];
  ...
  *decode_result = NULL;
  info->redirect_count = 0; // for counting the redirect times
  #if 0
  for(i = 0; i < strlen(info->url); i++)
  {
  	if (info->url[i] == '/') counts++;
	if (counts == 3) break;
  }
  urlonly = (char *)calloc(i, sizeof(char));
  if (urlonly == NULL) return -1;
  memcpy(urlonly, info->url, i);
  #endif
  
  do {
    maxlen = sizeof(buffer);
    buflen = snprintf(buffer, maxlen, "OPTIONS %s RTSP/1.0\r\n", /*urlonly*/info->url/*":554"*/);
    ...
    rtsp_debug(LOG_INFO, "Sending OPTIONS %s%s", /*urlonly*/info->url/*, ":554"*/);
    rtsp_debug(LOG_DEBUG, "%s", buffer);
    ///free(urlonly);
    ...
    if (ret == RTSP_RESPONSE_GOOD) {
      rtsp_debug(LOG_INFO, "OPTIONS returned correctly");
      *decode_result = info->decode_response;
      info->decode_response = NULL;
      return (RTSP_RESPONSE_GOOD);
    } else if (ret != RTSP_RESPONSE_REDIRECT) {
      rtsp_debug(LOG_ERR, "OPTIONS return code %d", ret);
      if (ret != RTSP_RESPONSE_RECV_ERROR &&
	  decode != NULL) {
	*decode_result = info->decode_response;
	info->decode_response = NULL;
  ...
  if (ret == RTSP_RESPONSE_GOOD) {
    ...
    *decode_result = info->decode_response;
	...
	return (RTSP_RESPONSE_RECV_ERROR);
      ...
      if ((*decode_result)->session != NULL)
      {
	// 2005.04.15 Sky add : need to notice the timeout value if the server has sent it to client
	// 			we need to send back the ACK (usually use GET_PARAMETER method) in the time
	int i = 0;
	int sessiontimeout = 0;
	for(i = 0; i < strlen((*decode_result)->session); i++)
	{
		if ((*decode_result)->session[i] == ';')
			break;
	}
	
	if (i + 8 < strlen((*decode_result)->session))
		sscanf((*decode_result)->session + (i + 1), "timeout=%d", &info->session_timeout);
	///printf("session_timeout : %d\n", info->session_timeout);
	(*decode_result)->session[i] = 0; // temperaily save 0 to prevent strdup the timeout value
	sptr->session = (char *)strdup((*decode_result)->session);
	(*decode_result)->session[i] = ';'; // save back the ';'
	// 2005.04.15 End of addition and modification
      }
      else
	sptr->session = NULL;
      sptr->parent = info;
