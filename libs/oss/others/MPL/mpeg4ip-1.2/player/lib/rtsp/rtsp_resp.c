...
// 2005.04.06 Sky add : for microsoft rtsp protocol
// #ifdef ENABLE_WMA
RTSP_HEADER_FUNC(rtsp_header_xplaylistgenid)
{
  DEC_DUP_WARN(x_playlist_gen_id, "X-Playlist-Gen-Id");
}

RTSP_HEADER_FUNC(rtsp_header_xbroadcastid)
{
  DEC_DUP_WARN(x_broadcast_id, "X-Broadcast-Id");
}
// #endif
// 2005.04.06 End of add

// 2005.08.16 Sky add : for realaudio rtsp protocol
// #ifdef ENABLE_RA
RTSP_HEADER_FUNC(rtsp_header_realchallenge1)
{
  DEC_DUP_WARN(realchallenge1, "RealChallenge1");
}

RTSP_HEADER_FUNC(rtsp_header_statsmask)
{
  DEC_DUP_WARN(statsmask, "StatsMask");
}

RTSP_HEADER_FUNC(rtsp_header_etag)
{
  DEC_DUP_WARN(etag, "Etag");
}
// #endif
// 2005.08.16 End of add
...
#define HEAD_TYPE(a, b) { a, sizeof(a) - 1, b }
  ...
  // #ifdef ENABLE_WMA
  HEAD_TYPE("X-Playlist-Gen-Id", rtsp_header_xplaylistgenid),	// 2005.04.06 Sky add : for microsoft rtsp protocol
  HEAD_TYPE("X-Broadcast-Id", rtsp_header_xbroadcastid),	// 2005.04.06 Sky add : for microsoft rtsp protocol
  // #endif
  // #ifdef ENABLE_RA
  HEAD_TYPE("RealChallenge1", rtsp_header_realchallenge1),	// 2005.08.16 Sky add : for realaudio rtsp protocol
  HEAD_TYPE("StatsMask", rtsp_header_statsmask),		// 2005.08.16 Sky add : for realaudio rtsp protocol
  HEAD_TYPE("Etag", rtsp_header_etag),				// 2005.08.16 Sky add : for realaudio rtsp protocol
  // #endif
  { NULL, 0, NULL },
};
...
static int rtsp_save_and_read (rtsp_client_t *cptr)
{
  int last_on;
  int ret;
  // 1. reduce the m_buffer_len (last length)
  cptr->m_buffer_len -= cptr->m_offset_on;
  // 2. a. last length >= RECV_BUFF_DEFAULT_LEN ==> just return 0
  //    b. last length < RECV_BUFF_DEFAULT_LEN but not equal 0 ==> memmove to the start of array
  //	c. last length == 0 ==> last_on = 0;
				       ...
				       const char *seperator)
{
   ...
   */
  // 2005.11.29 : bug fix ==> re-search the buffer starting from the "cptr->m_offset_on"
  #if 0
...
int rtsp_get_response (rtsp_client_t *info)
{
  ...
  while (1) {
	...
	return (RTSP_RESPONSE_RECV_ERROR);
      ...
      ///printf("allocation for decode_response in the info ok\n");
    }
