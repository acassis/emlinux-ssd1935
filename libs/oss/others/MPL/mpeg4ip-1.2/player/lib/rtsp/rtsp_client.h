...
typedef struct rtsp_command_t {
  ...
  char *range;
  char *referer;
  /// 2005.03.31 Sky modify : change type from double to the char speed[20] to prevent use the float point
  #if 0
  double scale;
  #else
  char scale[20];
...
typedef struct rtsp_decode_t {
  ...
  // #ifdef ENABLE_WMA
  char *x_playlist_gen_id; // 2005.04.06 Sky add : for microsoft ms rtsp
  char *x_broadcast_id;	   // 2005.04.06 Sky add : for microsoft ms rtsp
  // #endif
  // #ifdef ENABLE_RA
  char *realchallenge1;	   // 2005.08.16 Sky add : for realaudio ra rtsp
  char *statsmask;	   // 2005.08.16 Sky add : for realaudio ra rtsp
  char *etag;		   // 2005.08.16 Sky add : for realaudio ra rtsp
  // #endif
} rtsp_decode_t;
...
// 2005.08.16 Sky defines : TODO minimize code size
