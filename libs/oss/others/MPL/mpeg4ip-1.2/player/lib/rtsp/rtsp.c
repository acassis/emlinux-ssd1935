...
rtsp_client_t *rtsp_create_client_common (const char *url, int *perr)
{
  ...
  info->pUMSpecial = NULL; // 2006.01.02 special pointer to special structure of vendor
  info->next_cseq = 1;
  info->session = NULL;
  info->m_offset_on = 0;
  info->m_buffer_len = 0;
  ...
  info->thread = NULL;
  info->session_timeout = 0; // 2004.04.15 Sky add : session timeout default value is 0 means no timeout
