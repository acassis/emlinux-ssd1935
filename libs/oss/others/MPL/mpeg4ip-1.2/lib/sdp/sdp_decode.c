				     ...
				     media_desc_t *mptr)
{
  ...
  #else
  // TO DO : ... 2005.04.12 Sky modify : we don't want use floating point so ..
  #endif
  mptr->framerate_present = TRUE;
  return (0);
}
// 2005.04.25 Sky modify : for use char string to replace the double type
#if 0
static int convert_npt (char *from, char *to, double *ret)
#else
static int convert_npt (char *from, char *to, char **ret)
#endif
// 2005.02.25 End of modification
{
  int decimal = FALSE;
  
  // 2005.04.25 Sky modification : we don't support floating point, so we need use
  // 				another way to extract the range out
  #if 0
  double accum;
  double mult;
  ...
  *ret += accum;
  ...
  #endif
  // 2005.04.25 End of modification
...
#else
static int convert_smpte (char *from, char *to, uint16_t fps, char **ret)
#endif
// 2005.04.25 End of modification
{
  int decimal = FALSE;
...
// 2005.03.14 Sky add : just add the MS parameter pgmpu to USER for parse the pgmpu later in the calling process
#if 0
static int sdp_decode_parse_a_pgmpu (int arg,
				   char *lptr,
				   session_desc_t *sptr,
				   media_desc_t *mptr)
{
  pgmpu_data_list *pgmpu_data_list_p = NULL;
  
  // belong to media_desc_t we use the media_desc_t USER pointer, 
  // otherwise we use the session_desc_t USER pointer
  if (mptr == NULL)
    pgmpu_data_list_p = (pgmpu_data_list *)sptr->USER;
  else
    pgmpu_data_list_p = (pgmpu_data_list *)mptr->USER;
  
  if (pgmpu_data_list_p == NULL)
  {
    pgmpu_data_list_p = (pgmpu_data_list *)calloc(1, sizeof(pgmpu_data_list));
    if (pgmpu_data_list_p == NULL) return -1;
    // assign back to the sptr->USER pointer (The start of the pgmpu first data)
    sptr->USER = pgmpu_data_list_p;
  }
  else
  {
    // find the ->next is NULL
    while (pgmpu_data_list_p->next != NULL)
      pgmpu_data_list_p = pgmpu_data_list_p->next;
    // new one
    pgmpu_data_list_p->next = (pgmpu_data_list *)calloc(1, sizeof(pgmpu_data_list));
    if (pgmpu_data_list_p == NULL) return -1;
    // assign
    pgmpu_data_list_p = pgmpu_data_list_p->next;
  }
  // save the pgmpu data
  pgmpu_data_list_p->data = strdup(lptr);
  pgmpu_data_list_p->start = pgmpu_data_list_p; // bug TODO : need fix
  pgmpu_data_list_p->next = NULL;

  return (0);
}
#endif
// 2005.03.14 End of addition

static int sdp_decode_parse_a_door (char *type_name, 
				   int arg,
				   char *lptr,
				   session_desc_t *sptr,
				   media_desc_t *mptr)
{
  ...
  ///printf("type name : %s\n", type_name);
  // if belong to media_desc_t, we use the media_desc_t USER pointer, 
  // otherwise we use the session_desc_t USER pointer
  if (mptr == NULL)
    pgmpu_data_list_p = (pgmpu_data_list *)sptr->USER;
  else
  ...
  // save the pgmpu data and name
  memcpy(pgmpu_data_list_p->a_name, type_name, strlen(type_name));
  pgmpu_data_list_p->data = strdup(lptr);
  pgmpu_data_list_p->start = sptr->USER; // bug TODO : need fix
  pgmpu_data_list_p->next = NULL;
...
static int sdp_decode_parse_a_pgmpu (int arg,
				   ...
				   media_desc_t *mptr)
{
  return sdp_decode_parse_a_door("pgmpu", arg, lptr, sptr, mptr);
}

static int sdp_decode_parse_a_RMFF_1_0_Flags (int arg,
				   char *lptr,
				   session_desc_t *sptr,
				   media_desc_t *mptr)
{
  return sdp_decode_parse_a_door("RMFF 1.0 Flags", arg, lptr, sptr, mptr);
}

static int sdp_decode_parse_a_OpaqueData (int arg,
				   char *lptr,
				   session_desc_t *sptr,
				   media_desc_t *mptr)
{
  return sdp_decode_parse_a_door("OpaqueData", arg, lptr, sptr, mptr);
}

static int sdp_decode_parse_a_ASMRuleBook (int arg,
				   char *lptr,
				   session_desc_t *sptr,
				   media_desc_t *mptr)
{
  return sdp_decode_parse_a_door("ASMRuleBook", arg, lptr, sptr, mptr);
}
...
} a_types[] =
{
  ...
  ///#ifdef ENABLE_WMA
  { "pgmpu", sizeof("pgmpu"), TRUE, TRUE, sdp_decode_parse_a_pgmpu, 0 }, // 2005.03.14 Sky add for MS parameter pgmpu
  ///#endif
  ///#ifdef ENABLE_RA
  { "RMFF 1.0 Flags", sizeof("RMFF 1.0 Flags"), TRUE, TRUE, sdp_decode_parse_a_RMFF_1_0_Flags, 0 }, // 2005.03.14 Sky add for realaudio parameter a
  { "OpaqueData", sizeof("OpaqueData"), TRUE, TRUE, sdp_decode_parse_a_OpaqueData, 0 }, // 2005.03.14 Sky add for realaudio parameter a
  { "ASMRuleBook", sizeof("ASMRuleBook"), TRUE, TRUE, sdp_decode_parse_a_ASMRuleBook, 0 }, // 2005.03.14 Sky add for realaudio parameter a
  ///#endif
...
static int sdp_decode_parse_origin (char *lptr, session_desc_t *sptr)
{
  ...
  // 2005.09.19 Sky mark off : don't skip 0x20 here
  //ADV_SPACE(lptr);
  ...
  // 2005.09.19 Sky add : advance the lptr for one position if 0x20 meets
  if (isspace(*(lptr)) && (*(lptr) != '\0')) lptr++;
  else
  {
