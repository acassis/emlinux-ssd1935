...
// 2005.03.14 Sky add : MS parameter pgmpu for parse the pgmpu later in the calling process
typedef struct _pgmpu_data_list
{
  //#ifdef ENABLE_RA
  char a_name[32];
  //#endif
  void *data;
  struct _pgmpu_data_list *next;
  struct _pgmpu_data_list *start;
} pgmpu_data_list;
// 2005.03.14 End of add
...
typedef struct range_desc_t {
  ...
  int have_range;
  int range_is_npt;
  // 2005.04.25 Sky modify : don't use double type
  #if 0
  double range_start;
  double range_end;
  #else
...
typedef struct media_desc_t {
  ...
  uint32_t quality;
  int quality_present;
  // 2005.04.12 Sky modify : use the char to replace the double
  #if 0
  double framerate;
  #else
  char framerate[20];
...
#else
void sdp_smpte_to_str(char *value, uint16_t fps, char *buffer);
#endif
// 2005.04.25 End of modification
